...
- (NSArray *)searchPage:(NSUInteger)page withSelection:(BOOL)withSelection {
    PSPDFLogVerbose(@"Extracting text from %@, page %lu withSelection:%d", document_.title, (unsigned long)page, withSelection);
...
// thread entry point
- (void)main {    
    [delegate_ willStartSearchOperation:self forString:self.searchText isFullSearch:searchPages_ == nil];
    ...
    [selectionSearchPages_ enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        [self.delegate didUpdateSearchOperation:self forString:self.searchText newSearchResults:results forPage:page];
    }];
    ...
    if (searchPages_) {
        ...
        for (int page = 0; page < [document_ pageCount]; page++) {
            ...
            [delegate_ didUpdateSearchOperation:self forString:searchText_ newSearchResults:results forPage:page];
        }
    }
    ...
    if (selectionSearchPages_ != selectionSearchPages) {
        ...
        [selectionSearchPages_ enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            if ([pageNr integerValue] >= self.document.pageCount) {
                PSPDFLogError(@"Page Number out of range: %ld (count: %lu)", (long)[pageNr integerValue], (unsigned long)self.document.pageCount);
            }
        }];
    }
}
