...
- (void)setWidthsWithFontDictionary:(CGPDFDictionaryRef)dict
{
	...
	for (int i = 0; i < count; )
	{
		...
		if (type == kCGPDFObjectTypeInteger)
		{
			...
			for (long int index = firstCharacter; index <= lastCharacter; index++)
			{
				NSNumber *key = [NSNumber numberWithLong:index];
				NSNumber *val = [NSNumber numberWithLong:characterWidth];
		...
		else if (type == kCGPDFObjectTypeArray)
		{
			...
			for (int index = 0; index < widthsCount ; index++)
			{
				...
				{
					NSNumber *key = [NSNumber numberWithLong:firstCharacter+index];
					NSNumber *val = [NSNumber numberWithLong:width];
					[widthsDict setObject:val forKey:key];
				}
			}
		}
