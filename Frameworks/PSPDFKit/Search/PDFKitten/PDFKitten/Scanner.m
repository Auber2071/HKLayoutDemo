...
@property (nonatomic, strong) Selection *currentSelection;
@property (ps_weak, nonatomic, readonly) RenderingState *currentRenderingState;
@property (ps_weak, nonatomic, readonly) Font *currentFont;
...
- (CGPDFDocumentRef)pdfDocument
{
	if (!pdfDocument)
	{
		pdfDocument = CGPDFDocumentCreateWithURL((__bridge CFURLRef)self.documentURL);
	}
	return pdfDocument;
}
...
- (FontCollection *)fontCollectionWithPage:(CGPDFPageRef)page
{
	...
	if (!dict)
	{
		PSPDFLogWarning(@"Scanner: fontCollectionWithPage: page dictionary missing");
		return nil;
	}
	CGPDFDictionaryRef resources;
	if (!CGPDFDictionaryGetDictionary(dict, "Resources", &resources))
	{
		PSPDFLogWarning(@"Scanner: fontCollectionWithPage: page dictionary missing Resources dictionary");
		return nil;	
	}
...
{
    NSAssert(page != nil, @"PageRef must not be nil!");
    if(page == nil) {
        PSPDFLogError(@"Scanner initialized with nil pageRef.");
        return;
    }
...
void BT(CGPDFScannerRef scanner, void *info)
{
	[[(__bridge Scanner *)info currentRenderingState] setTextMatrix:CGAffineTransformIdentity replaceLineMatrix:YES];
}
...
void didScanSpace(float value, Scanner *scanner)
{
    ...
    if (fabsf(value) >= [scanner.currentRenderingState.font widthOfSpace])
    {
...
void didScanString(CGPDFStringRef pdfString, Scanner *scanner)
{
	...
	if (string && scanner.rawTextContent)
{
	...
	for (int i = 0; i < count; i++)
	{
        ...
        switch (type)
        {
            ...
            case kCGPDFObjectTypeReal:
            {
                ...
                didScanSpace(tx, (__bridge Scanner *)info);
                break;
            }
            ...
            default:
                PSPDFLogWarning(@"Scanner: TJ: Unsupported type: %d", type);
                break;
        }
	}
}
...
void Td(CGPDFScannerRef scanner, void *info)
{
	...
	[[(__bridge Scanner *)info currentRenderingState] newLineWithLeading:-ty indent:tx save:NO];
}
...
void TD(CGPDFScannerRef scanner, void *info)
{
	...
	[[(__bridge Scanner *)info currentRenderingState] newLineWithLeading:-ty indent:tx save:YES];
}
...
void Tm(CGPDFScannerRef scanner, void *info)
{
	...
	[[(__bridge Scanner *)info currentRenderingState] setTextMatrix:t replaceLineMatrix:YES];
}

/* Go to start of new line, using stored text leading */
void TStar(CGPDFScannerRef scanner, void *info)
{
	[[(__bridge Scanner *)info currentRenderingState] newLine];
}
...
void Tc(CGPDFScannerRef scanner, void *info)
{
	...
	[[(__bridge Scanner *)info currentRenderingState] setCharacterSpacing:charSpace];
}
...
void Tw(CGPDFScannerRef scanner, void *info)
{
	...
	[[(__bridge Scanner *)info currentRenderingState] setWordSpacing:wordSpace];
}
...
void Tz(CGPDFScannerRef scanner, void *info)
{
	...
	[[(__bridge Scanner *)info currentRenderingState] setHorizontalScaling:hScale];
}
...
void TL(CGPDFScannerRef scanner, void *info)
{
	...
	[[(__bridge Scanner *)info currentRenderingState] setLeadning:leading];
}
...
void Tf(CGPDFScannerRef scanner, void *info)
{
	...
	RenderingState *state = [(__bridge Scanner *)info currentRenderingState];
	Font *font = [[(__bridge Scanner *)info fontCollection] fontNamed:[NSString stringWithUTF8String:fontName]];
	[state setFont:font];
...
void Ts(CGPDFScannerRef scanner, void *info)
{
	...
	[[(__bridge Scanner *)info currentRenderingState] setTextRise:rise];
}
...
void q(CGPDFScannerRef scanner, void *info)
{
	RenderingStateStack *stack = [(__bridge Scanner *)info renderingStateStack];
	RenderingState *state = [[(__bridge Scanner *)info currentRenderingState] copy];
...
void Q(CGPDFScannerRef scanner, void *info)
{
	[[(__bridge Scanner *)info renderingStateStack] popRenderingState];
}
