...
//		PSPDFLogVerbose(@"%d,%d offset %d", start, end, offset);
...
- (void)scanChars:(NSScanner *)aScanner 
{
	NSString *content = nil;
	static NSString *endToken = @"endbfchar";
	[aScanner scanUpToString:endToken intoString:&content];
    ...
    while (![rangeScanner isAtEnd])
    {
        ...
        line = [line stringByReplacingOccurrencesOfString:@" " withString:@""]; // remove any whitespace
        ...
        NSNumber *fromNumber = [NSNumber numberWithUnsignedInt:from];
        ...
        if ([toString length] > charLen) {
            ...
            for (int offset = 0; offset < [toString length]/4; offset++) {
                ...
                [toArray addObject: [NSNumber numberWithUnsignedInt:to]];
            }
        ...
        else 
        {
            ...
            [scanner scanHexInt:&to];
            NSNumber *toNumber = [NSNumber numberWithUnsignedInt:to];
...
- (void)scanning:(NSString *)text
{
	...
	PSPDFLogVerbose(@"%lu", (unsigned long)scanner.scanLocation);
...
- (NSString *)unicodeCharacter:(unichar)cid
{
    ...
    if (dict)
    {
        ...
        result = [NSString stringWithFormat:@"%ld", (long)[[dict objectForKey:@"Offset"] integerValue] + internalOffset];
    }
    else if (chars)
    {
    ...
    if (!result) {
        result = [NSString stringWithFormat: @"%C", cid];
    }
