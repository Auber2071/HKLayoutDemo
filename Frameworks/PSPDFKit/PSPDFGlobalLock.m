...
- (CGPDFPageRef)openPDFWithDocument:(PSPDFDocument *)document page:(NSUInteger)page error:(NSError **)error {
    ...
    PSPDFLogVerbose(@"path: %@, page:%lu", pdfPath, (unsigned long)pdfPageNumber);
    ...
    if (pdfDocument_) {
        ...
        if (!pdfPage_) {
            PSPDFLogError(@"pdf page reference could not be aquired for %@, page %lu (Maybe the PDF is corrupted, locked or just doesn't exist)", [pdfPath absoluteString], (unsigned long)page);
            
            if(error) {
                NSDictionary *errorDict = [NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Failed to open the pdf %@, page %lu. (Maybe the PDF is corrupted, locked or just doesn't exist)", [pdfPath absoluteString], (unsigned long)page] forKey:NSLocalizedDescriptionKey];
