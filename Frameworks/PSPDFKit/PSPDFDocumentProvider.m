...
- (CGPDFPageRef)requestPageRefForPage:(NSUInteger)page {
    ...
    pageRef = (__bridge CGPDFPageRef)[openPages_ objectForKey:[NSNumber numberWithInteger:page]];
#endif
    if (!pageRef) {
        pageRef = CGPDFDocumentGetPage(documentRef_, page);
        if (!pageRef) {
            PSPDFLogWarning(@"Error while getting pageRef for page %lu", (unsigned long)page);
        }else {
