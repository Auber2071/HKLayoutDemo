...
- (NSString *)description {
    return [NSString stringWithFormat:@"<PSPDFPageInfo rect:%@ rotation:%lu>", NSStringFromCGRect(self.pageRect), (unsigned long)self.pageRotation];
}
...
- (CGRect)rotatedPageRect {
    NSInteger pageRotation = self.pageRotation;
...
- (NSString *)description {
    NSString *description = [NSString stringWithFormat:@"<PSPDFPageCoordinates pdfPoint:%@ screenPoint:%@ viewPoint:%@ pageSize:%@ zoomScale:%.2f>", NSStringFromCGPoint(pdfPoint_), NSStringFromCGPoint(screenPoint_), NSStringFromCGPoint(viewPoint_), NSStringFromCGSize(pageSize_), zoomScale_];
    return description;
}
