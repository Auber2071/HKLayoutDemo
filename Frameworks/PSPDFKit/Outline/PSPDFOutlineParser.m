...
@synthesize firstVisibleElement = firstVisibleElement_;
...
- (NSArray *)parseOutlineElements:(CGPDFDictionaryRef)outlineElementRef level:(NSUInteger)level error:(NSError **)error_  documentRef:(CGPDFDocumentRef)documentRef cache:(NSMutableArray *)cache {
    ...
    PSPDFOutlineElement *outlineElement = [[PSPDFOutlineElement alloc] initWithTitle:outlineTitle page:(pageIndex >=0 ? pageIndex : 1) children:descendantOutlineElements level:level];
...
- (PSPDFOutlineElement *)outline {
    if (!outline_) {
        ...
        outline_ = [[PSPDFOutlineElement alloc] initWithTitle:nil page:0 children:outlineElements level:0];
    }
    return outline_;
}
    ...
    if(CGPDFDictionaryGetDictionary(pdfDocDictionary, "Outlines", &outlineRef)) {
        ...
        if ([namedDestinations_ count] > 0) {
            ...
            [resolvedNames enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
                PSPDFOutlineElement *outlineElement = [self.namedDestinations objectForKey:key];
                if(outlineElement) {
                    NSInteger destPage = [obj integerValue];
                    PSPDFLogVerbose(@"MATCH FOUND: %@ (%@) is page %ld", key, outlineElement.title, (long)destPage);
