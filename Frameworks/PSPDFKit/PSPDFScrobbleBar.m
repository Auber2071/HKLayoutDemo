...
- (CGFloat)leftPosForThumbSize:(CGSize)thumbSize page:(NSUInteger)page {
    ...
    CGFloat thumbWidth = kPSPDFScrobbleThumbSize.width;
    ...
    // map the images pixel-perfect
    /*
     if (page > 0 && page < [self.pdfController.document pageCount]) {
     UIImageView *imageView = [imageViews_ objectForKey:[NSNumber numberWithInteger:page]];
     if (imageView) {
     CGFloat newMarkerPos = floorf(imageView.frame.origin.x + (kPSPDFScrobbleThumbSize.width - thumbSize.width)/2.f);
...
- (void)updateToolbarPositionAnimated:(BOOL)animated forced:(BOOL)forced {
    ...
    if (shouldShow || shouldHide) {
        ...
        [UIView animateWithDuration:animated ? 0.25f : 0.f delay:0.f options:UIViewAnimationOptionAllowUserInteraction animations:^{
            if (shouldShow) {
                self.alpha = 1.f;
                self.frame = CGRectMake(0, self.pdfController.view.frame.size.height-kPSPDFScrobbleBarHeight, self.pdfController.view.frame.size.width, kPSPDFScrobbleBarHeight);
            }else {
                self.alpha = 0.f;
                self.frame = CGRectMake(0, self.pdfController.view.frame.size.height, self.pdfController.view.frame.size.width, kPSPDFScrobbleBarHeight);
            }
            self.viewLocked = NO;
        } completion:nil];
    }
...
- (CGRect)rectForThumb:(CGSize)thumbSize withImageSize:(CGSize)imageSize position:(NSInteger)position {
    ...
    if (!CGSizeEqualToSize(imageSize, CGSizeZero)) {
        CGFloat scale = PSPDFScaleForSizeWithinSize(imageSize, kPSPDFScrobbleThumbSize);
        ...
        if (!dualPageMode || ABS(lastTouchedPage_ - page) > 1 ||
            ((lastTouchedPage_ > page && !(dualPageMode && [self.pdfController isRightPageInDoublePageMode:page+1])) ||
             (lastTouchedPage_ < page && !(dualPageMode && [self.pdfController isRightPageInDoublePageMode:page])))) {
                PSPDFLogVerbose(@"scrolling to page: %lu", (unsigned long)page);
                processed = [self.pdfController scrollToPage:page animated:(animated && PSPDFShouldAnimate()) hideHUD:NO];
            }
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    lastTouchedPage_ = page;
    [self setPageInternal:lastTouchedPage_];
    //TODO disable tiny scrollablebar preview
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if (context == &kPSPDFKVOToken) {
        ...
        if (!self.hidden) {
            [self setPage:_pdfController.realPage];
            [self updatePageMarker];
...
- (void)setPdfController:(PSPDFViewController *)pdfController {
    if(pdfController != _pdfController) {
        ...
        toolbar_.barStyle = [pdfController isDarkHUD] ? UIBarStyleBlackOpaque : UIBarStyleDefault;
...
- (void)updateToolbarForced {
    ...
    PSPDFLogVerbose(@"Showing %lu thumbnails.", (unsigned long)thumbCount_);

    // build reversed, to have proper caching requests in FIFO stack
    NSInteger lastPage = pageCount;
    for (NSInteger i = thumbCount_-1; i >= 0; i--) {
...
- (void)didCachePageForDocument:(PSPDFDocument *)document page:(NSUInteger)page image:(UIImage *)cachedImage size:(PSPDFSize)size {
    ...
    if (self.window && size == PSPDFSizeTiny && document == self.pdfController.document) {
        ...
        if (imageView) {
            ...
            imageView.frame = [self rectForThumb:kPSPDFScrobbleThumbSize withImageSize:cachedImage.size position:imageView.tag - kPSPDFPageImagesTagOffset];
        }
